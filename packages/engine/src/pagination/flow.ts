  ...
  switch (block.kind) {
    case 'text': {
      ...
      return {
        ...
        remaining:
          placed.nextLine === undefined ? undefined : { kind: 'text', line: placed.nextLine },
      };
    }
    case 'image': {
      const height = metrics.height(block.key);
      if (height > fresh) {
        throw refusal(IMAGE_TOO_TALL, 'oversized-atomic-resource', {
          nodeId: block.nodeId,
          path: block.path,
        });
      }
    ...
    case 'container': {
      ...
      if (block.children.length === 0) {
        const height = metrics.height(block.key);
        if (height > fresh) {
          throw refusal(BOX_TOO_TALL, 'pagination-impossible', {
            nodeId: block.nodeId,
            path: block.path,
          });
        }
        return height > available
          ? undefined
          : { fragment: wholeFragment(block), height, remaining: undefined };
      }
      if (fresh - padding <= 0) {
      ...
      return {
        ...
        fragment: {
          ...
          edge:
            from.index === 0 && from.inner === undefined
              ? done
                ? 'whole'
                : 'first'
              : done
